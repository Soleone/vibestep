  ...
  return (
    <main className={activeTab === 'editor' ? 'editing-layout' : undefined}>
      ...
      <aside className="panel">
        ...
        {activeTab === 'editor' && <>
          {beatmapControls}
          <Card><CardHeader><CardTitle>Run feedback</CardTitle><CardDescription>{reviewNoteResults.size > 0 ? 'Each current note shows its newest valid result across your play runs.' : 'Play this beatmap to collect note timing feedback.'}</CardDescription></CardHeader>{reviewNoteResults.size > 0 && <><div className="metric-row"><Badge tone="success">Perfect {reviewCounts.perfect}</Badge><Badge tone="warning">Good {reviewCounts.good}</Badge><Badge tone="danger">Needs work {reviewCounts.missed}</Badge></div><p className="run-review-hint"><span>← early</span><span>late →</span><span>× no input</span></p></>}{lastPlayRun && <Button type="button" variant="ghost" size="sm" onClick={() => { if (window.confirm('Discard the most recent run and remove its editor feedback?')) void deletePlayRun(lastPlayRun.id) }}><Trash2 />Discard most recent run</Button>}</Card>
        </>}
