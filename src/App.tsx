...
function App() {
  ...
  const [quickstartOpen, setQuickstartOpen] = useState(() => readAppStorage('quickstart-seen-v1') !== 'true')
  ...
  const importYoutube = useCallback(async () => {
    ...
    if (companionStatus !== 'paired') {
      setImportStatus('Connect the companion to import YouTube audio.')
      setCompanionDialogOpen(true)
      return
    }
  ...
  const exportSharedBeatmap = useCallback(async () => {
    ...
    try {
      ...
      const slug = `${songPackage.song.title}-${selectedMap.title}`.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '') || `${appBrand.slug}-beatmap`
      downloadJson(bundle, `${slug}.${appBrand.slug}.json`)
  ...
  const importSharedBundle = useCallback(async (file: File) => {
    try {
      ...
      if (bundle.songs.length !== 1 || bundle.songs[0].beatmaps.length !== 1) throw new Error(`Choose a single-song, single-difficulty ${appBrand.name} export`)
    ...
    <main className={activeTab === 'editor' ? 'editing-layout' : 'play-layout'}>
      ...
      <QuickstartDialog
        ...
        onOpenChange={(open) => {
          ...
          if (!open) localStorage.setItem(storageKey('quickstart-seen-v1'), 'true')
        }}
        onOpenSettings={() => {
          localStorage.setItem(storageKey('quickstart-seen-v1'), 'true')
      ...
      <Dialog open={companionDialogOpen} onOpenChange={setCompanionDialogOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{companionStatus === 'permission-blocked' ? 'Allow companion access' : 'Connect the companion'}</DialogTitle>
            <DialogDescription>{companionStatus === 'permission-blocked' ? `Chrome has blocked ${appBrand.name} from contacting software on this computer.` : 'The companion is optional. Browser file imports continue to work without it.'}</DialogDescription>
          </DialogHeader>
          {companionStatus === 'permission-blocked'
            ...
            : <div className="companion-dialog-copy"><p>{appBrand.name} needs permission to contact the companion running only on this computer.</p><p>Your browser may ask you to allow <strong>Apps on device</strong>. {appBrand.name} uses this access only for the local companion.</p></div>}
          <DialogFooter>
      ...
      <aside className="panel" inert={settingsOpen}>
        <div className="panel-hero"><AppBrand /></div>
